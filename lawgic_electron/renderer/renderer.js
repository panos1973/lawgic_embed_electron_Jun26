...
// ---- show/hide ("eye") toggle on every API-key field in Settings ----
// Injected in JS so it covers all current key inputs (and any future one) without
// editing each label. Idempotent: re-running won't double-wrap.
const _EYE = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" '
  + 'stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">'
  + '<path d="M1 12s4-7 11-7 11 7 11 7-4 7-11 7-11-7-11-7z"/><circle cx="12" cy="12" r="3"/></svg>';
const _EYE_OFF = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" '
  + 'stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">'
...
function addKeyEyes() {
  document.querySelectorAll('#settings input[type="password"]').forEach((inp) => {
