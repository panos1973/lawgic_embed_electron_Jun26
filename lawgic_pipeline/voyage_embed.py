...
# scanned/image-only gazette page Azure DI returned empty for, a structure-only section)
...
def embed_law_chunks(ordered_chunks: list[str],
    ...
    # 0 in your batch has ..."), which fails the WHOLE document. A blank provision (a
    # scanned/image-only gazette page DI returned empty for, a structure-only section)
    # carries no text -> substitute a placeholder so the document still embeds and loads.
